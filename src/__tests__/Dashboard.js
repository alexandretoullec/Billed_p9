...
describe("Given I am a user connected as Admin", () => {
  describe("When I navigate to Dashboard", () => {
    ...
    test("fetches bills from mock API GET", async () => {
      ...
      // Call the router function to navigate to the Dashboard
      router();
      window.onNavigate(ROUTES_PATH.Dashboard);

      // Wait for the "Validations" text to be present on the screen
      await waitFor(() => screen.getByText("Validations"));

      // Check if content for "En attente (1)" is present
      const contentPending = await screen.getByText("En attente (1)");
      expect(contentPending).toBeTruthy();

      // Check if content for "Refusé (2)" is present
      const contentRefused = await screen.getByText("Refusé (2)");
      expect(contentRefused).toBeTruthy();

      // Check if the "big-billed-icon" is present
      expect(screen.getByTestId("big-billed-icon")).toBeTruthy();
    });

    // Nested describe block for API error scenarios
    describe("When an error occurs on API", () => {
      beforeEach(() => {
        // Mock the store's "bills" method and set user type and email in localStorage
        ...
        // Create a root element and append it to the document body, then call the router function
        const root = document.createElement("div");
        root.setAttribute("id", "root");
      ...
      // Test case 2: Fetches bills from an API and fails with 404 message error
      ...
      // Test case 3: Fetches bills from an API and fails with 500 message error
